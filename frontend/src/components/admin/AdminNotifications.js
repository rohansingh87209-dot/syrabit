...
export default function AdminNotifications({ adminToken }) {
  ...
  useEffect(() => {
    axios.get(`${API_BASE}/admin/notifications`, { headers: adminToken ? { Authorization: `Bearer ${adminToken}` } : {}, withCredentials: true })
      .then((r) => setNotifs(r.data)).catch(() => {}).finally(() => setLoading(false));
  }, [adminToken]);
  ...
  const handleSend = async (status) => {
    ...
    setSending(true);
    try {
      const res = await axios.post(`${API_BASE}/admin/notifications`, { title, message, type, audience, status }, { headers: adminToken ? { Authorization: `Bearer ${adminToken}` } : {}, withCredentials: true });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${API_BASE}/admin/notifications/${id}`, { headers: adminToken ? { Authorization: `Bearer ${adminToken}` } : {}, withCredentials: true });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 max-w-5xl">
      ...
      <div className="lg:col-span-7 rounded-2xl border border-white/6 p-5 space-y-4" style={{ background: 'rgba(255,255,255,0.02)' }}>
        ...
        <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Notification title"
          className="w-full h-9 px-3 rounded-xl text-sm text-white outline-none" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.10)' }} />
        <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Message body..." rows={4}
          className="w-full p-3 rounded-xl text-sm text-white resize-none outline-none" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.10)' }} />
        {/* Type */}
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {NOTIF_TYPES.map(({id, icon: Icon, color, bg, border}) => (
              <button key={id} onClick={() => setType(id)}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium border transition-all ${type === id ? `${bg} ${border} ${color}` : 'border-white/8 text-white/40'}`}>
          ...
          <div className="space-y-1.5">
            {AUDIENCES.map(({id, label}) => (
              <button key={id} onClick={() => setAudience(id)}
                className={`w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-all border ${audience === id ? 'border-violet-500/30 bg-violet-500/10 text-violet-300' : 'border-white/6 text-white/40 hover:text-white/60 hover:bg-white/3'}`}>
        ...
        <div className="flex gap-2">
          <button onClick={() => handleSend('draft')} disabled={sending}
          ...
          <button onClick={() => handleSend('sent')} disabled={sending}
            className="flex-1 h-9 rounded-xl text-xs font-semibold text-white flex items-center justify-center gap-1.5" style={{ background: 'linear-gradient(135deg,#7c3aed,#8b5cf6)' }}>
      ...
      <div className="lg:col-span-5 rounded-2xl border border-white/6 overflow-hidden" style={{ background: 'rgba(255,255,255,0.02)' }}>
        <div className="p-4 border-b border-white/6 flex items-center justify-between">
          ...
          <div className="flex gap-1.5 text-[10px]">
            ...
            <span className="px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400">{notifs.filter((n) => n.status === 'sent').length} sent</span>
          </div>
        </div>
        <div className="overflow-y-auto" style={{ maxHeight: 480 }}>
          {notifs.length === 0 ? (
            <div className="text-center py-12 text-white/20 text-sm">No notifications yet</div>
          ) : notifs.map((n) => {
            const tc = NOTIF_TYPES.find((t) => t.id === n.type) || NOTIF_TYPES[0];
            const Icon = tc.icon;
            return (
              <div key={n.id} className={`p-3 border-b border-white/5 ${n.status === 'draft' ? 'border-l-2 border-l-amber-500/40' : ''} group`}>
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      ...
                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${n.status === 'sent' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'}`}>{n.status}</span>
                    </div>
                    <p className="text-[11px] text-white/50 truncate">{n.message}</p>
                    <p className="text-[10px] text-white/25 mt-0.5">→ {n.audience}</p>
                  </div>
                  <button onClick={() => handleDelete(n.id)} className="opacity-0 group-hover:opacity-100 p-1 text-white/30 hover:text-red-400">
                    <Trash2 size={12} />
                  </button>
                </div>
              </div>
